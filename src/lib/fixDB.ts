...
export default async (knex: Knex): Promise<void> => {
  ...
  await knex("o_agentDeploy").where("vendorId", "toonflow").update({
    model: null,
    vendorId: null,
  });
  const toonflowPath = path.join(u.getPath("vendor"), "toonflow.ts");
  if (fs.existsSync(toonflowPath)) fs.rmSync(toonflowPath, { force: true });
};
